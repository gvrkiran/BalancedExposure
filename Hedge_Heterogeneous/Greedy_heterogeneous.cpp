...
void Greedy(int NUM_TO_SELECT){
	...
	while(main_iterations < NUM_TO_SELECT+1){
		...
		for (int i=0; i<K; i++){
			...
			int reachable_num;
			for(int j = 1; j<=n ;j++){
				reachable_num = reachable_from_vertex(j, GfirstOut_a, Gout_a, reachable_cascade_a, reachable_a, reachable_b, a_reachable, b_reachable, common_reachable);
				new_reachable_created_a[j]+= reachable_num; 
				//cout << "vertex "<< j <<" in a balances " << reachable_num << " vertices" << endl;
				
				reachable_num = reachable_from_vertex(j, GfirstOut_b, Gout_b, reachable_cascade_b, reachable_b, reachable_a, b_reachable, a_reachable, common_reachable);
		...
		for (int i=0; i<K; i++){
			...
			best_a_b_reachable += reachable_from_pair(best_vertex_a, best_vertex_b, GfirstOut_a, Gout_a, reachable_cascade_a, reachable_a, GfirstOut_b, Gout_b, reachable_cascade_b, reachable_b, reachable_num_a, reachable_num_b, common_reachable);
		}
		...
		if(best_value_a == 0 && best_value_b == 0 && best_value_common == 0 && best_value_a_b == 0) {
