...
void Greedy(int NUM_TO_SELECT){
	...
	while(main_iterations < NUM_TO_SELECT+1){
		...
		if(best_value_a > best_value_b){
			//cout << "vertex "<< best_vertex_a <<" in team a will balance "<< (double)best_value_a/(double)K <<" vertices on average"<< endl;
			if(find(b_seeds.begin(), b_seeds.end(), best_vertex_a) != b_seeds.end() || main_iterations == NUM_TO_SELECT -1){
				for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
					if (it->second == best_vertex_a){
						cout << "--> seed  "<< it->first <<"  is added in the first campaign and balances "<< (double)best_value_a/(double)K <<" vertices on average"<< endl;
					}
				}
				...
				for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
					if (it->second == best_vertex_a){
						cout << "--> seed  "<< it->first <<"  is added to both campaigns and balances "<< (double)best_value_a/(double)K <<" vertices on average"<< endl;
					}
				}
				a_seeds.push_back(best_vertex_a);
				b_seeds.push_back(best_vertex_a);
				main_iterations+=2;
			}
		}
		else{
			...
			if(find(a_seeds.begin(), a_seeds.end(), best_vertex_b) != a_seeds.end() || main_iterations == NUM_TO_SELECT -1){
				for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
					if (it->second == best_vertex_b){
						cout << "--> seed  "<< it->first <<"  is added in the second campaign and balances "<< (double)best_value_b/(double)K <<" vertices on average"<< endl;
					}
				}
				b_seeds.push_back(best_vertex_b);
				main_iterations+=1;
				...
				for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
					if (it->second == best_vertex_b){
						cout << "--> seed  "<< it->first <<"  is added to both campaigns and balances "<< (double)best_value_b/(double)K <<" vertices on average"<< endl;
					}
				}
				b_seeds.push_back(best_vertex_b);
				a_seeds.push_back(best_vertex_b);
				main_iterations+=2;
			}
		}
	}
