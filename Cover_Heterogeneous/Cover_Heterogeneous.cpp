...
vector<int> a_initial_seeds;
vector<int> b_initial_seeds;
vector<int> a_seeds;
vector<int> b_seeds;
...
int reachable_from_vertex(int v, int *GfirstOut, int* Gout, bool* reachable_cascade_same, bool* reachable_same, bool* reachable_other){
    ...
    int num_of_reachable_vertices = 0;
    int num_of_reachable_vertices_2 = 0;
	...
	while(queueHead != queueTail){
		...
		if(!reachable_same[selected]){
		    ...
		    if(reachable_other[selected] && initially_unbalanced[selected]) num_of_reachable_vertices++;
		    if(reachable_other[selected]) num_of_reachable_vertices_2++;
		}
		// scan all vertices that follows selected
		for(int j = GfirstIn_full[selected]; j < GfirstIn_full[selected+1]; j++){
	    	// mark any new exposed vertices
	    	if(!reachable_same[Gin_full[j]]){
			    ...
			    if(reachable_other[Gin_full[j]] && initially_unbalanced[Gin_full[j]]) num_of_reachable_vertices++;
			    if(reachable_other[Gin_full[j]]) num_of_reachable_vertices_2++;
	...
	//if(num_of_reachable_vertices > num_of_reachable_vertices_2) cout << "reachable : " << num_of_reachable_vertices << ", rechable_2 : " << num_of_reachable_vertices_2<<endl;
    return num_of_reachable_vertices;
}
...
void Greedy(int NUM_TO_SELECT){
	...
	initially_unbalanced = new bool [n+1];
	...
	int initially_unalanced_counter = 0;
	int initially_unbalnced_balanced = 0;
	...
	while(main_iterations < NUM_TO_SELECT+1){
		...
		initially_unalanced_counter = 0;
		initially_unbalnced_balanced = 0;
		...
		for (int i=0; i<K; i++){
			...
			for (unsigned j=0; j<edges_sampled.size(); j+=2){
		  	...
		  	int reachable_num_a = reachable_from_initial_seeds(GfirstOut_a, Gout_a, reachable_cascade_a, reachable_a, a_initial_seeds);
			...
			int reachable_num_b = reachable_from_initial_seeds(GfirstOut_b, Gout_b, reachable_cascade_b, reachable_b, b_initial_seeds);
			
			for(int j = 0; j <= n; j++) initially_unbalanced[j] = false;
			for(int j = 1; j<=n ;j++){
				if((reachable_a[j] && !reachable_b[j]) || (!reachable_a[j] && reachable_b[j])){
					initially_unbalanced[j] = true;
					initially_unalanced_counter++;
				} 
			}
			
			/*int temp_counter = 0;
			for(int j = 1; j<=n ;j++){
				if(initially_unbalanced[j]) temp_counter++;
			}
			cout << "counter = "<< temp_counter <<", n = " << n <<endl;*/
			
			reachable_num_a = reachable_from_initial_seeds(GfirstOut_a, Gout_a, reachable_cascade_a, reachable_a, a_seeds);
			reachable_num_b = reachable_from_initial_seeds(GfirstOut_b, Gout_b, reachable_cascade_b, reachable_b, b_seeds);
			...
			for(int j = 1; j<=n ;j++){
				...
				if(reachable_a[j] && reachable_b[j] && initially_unbalanced[j]){
					initially_unbalnced_balanced++;
				} 
			}
			...
			if(main_iterations != NUM_TO_SELECT && (i+1)%100 == 0) cout << "Average unbalanced initially unbalanced vertices: " <<((double)initially_unbalnced_balanced)/((double)(i+1)) << endl;
		}
		...
		if(best_value_a > best_value_b){
			for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
				if (it->second == best_vertex_a){
					cout << best_vertex_a <<"--> seed  "<< it->first <<"  is selected in the first campaign and balances "<< (double)best_value_a/(double)K <<" vertices on average"<< endl;
				}
			}
			...
			for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
				if (it->second == best_vertex_b){
					cout << best_vertex_b <<"--> seed  "<< it->first <<"  is selected in the second campaign and balances "<< (double)best_value_b/(double)K <<" vertices on average"<< endl;
				}
			}
...
//     }
    ...
    while (file_seeds_a >> user_a){
		it = user2ID.find(user_a);
		if(it != user2ID.end()){
			a_seeds.push_back(it->second);
			a_initial_seeds.push_back(it->second);
		} 
		else{
    ...
    while(a_seeds.size() > 40){
    	a_seeds.pop_back();
    	a_initial_seeds.pop_back();
	} 
    ifstream file_seeds_b(file3);
	while (file_seeds_b >> user_b){
		it = user2ID.find(user_b);
		if(it != user2ID.end()){
			b_seeds.push_back(it->second);
			b_initial_seeds.push_back(it->second);
		} 
		else{
    ...
    while(b_seeds.size() > 40){
		b_seeds.pop_back();
    	b_initial_seeds.pop_back();
	} 
