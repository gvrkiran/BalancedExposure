...
int reachable_from_vertex(int v, int *GfirstOut, int* Gout, bool* reachable_cascade_same, bool* reachable_same, bool* reachable_other){
	...
	//if(num_of_reachable_vertices != num_of_reachable_vertices_2) cout << "!!!!!"<<endl;
    return num_of_reachable_vertices;
}
...
void Greedy(int NUM_TO_SELECT){
	...
	while(main_iterations < NUM_TO_SELECT+1){
		...
		for (int i=0; i<K; i++){
			...
			int reachable_num_a = reachable_from_initial_seeds(GfirstOut, Gout, reachable_cascade_a, reachable_a, a_initial_seeds);
			int reachable_num_b = reachable_from_initial_seeds(GfirstOut, Gout, reachable_cascade_b, reachable_b, b_initial_seeds);
			
			for(int j = 1; j<=n ;j++){
				if((reachable_a[j] && !reachable_b[j]) || (!reachable_a[j] && reachable_b[j])) initially_unbalanced[j] = true;
			}
/*			int temp_counter = 0;
			for(int j = 1; j<=n ;j++){
...
*/			
			reachable_num_a = reachable_from_initial_seeds(GfirstOut, Gout, reachable_cascade_a, reachable_a, a_seeds);
			reachable_num_b = reachable_from_initial_seeds(GfirstOut, Gout, reachable_cascade_b, reachable_b, b_seeds);
