...
void Greedy(int NUM_TO_SELECT){
	...
	while(main_iterations < NUM_TO_SELECT+1){
		for(int i = 0; i<=n; i++) new_reachable_created_a[i] = new_reachable_created_b[i] = new_reachable_created_common[i] = 0;
		...
		for (int i=0; i<K; i++){
			...
			for(int j = 1; j<=n ;j++){
				reachable_num = reachable_from_vertex(j, GfirstOut, Gout, reachable_cascade_a, reachable_a, reachable_b, reachable_num_a, reachable_num_b, common_reachable);
				new_reachable_created_a[j]+= reachable_num; 
				//cout << "vertex "<< j <<" in a balances " << reachable_num << " vertices" << endl;
				
				reachable_num = reachable_from_vertex(j, GfirstOut, Gout, reachable_cascade_b, reachable_b, reachable_a, reachable_num_b, reachable_num_a, common_reachable);
				new_reachable_created_b[j]+= reachable_num;
				//cout << "vertex "<< j <<" in b balances " << reachable_num << "vertices" << endl;
				
				reachable_num = reachable_from_pair(j, j, GfirstOut, Gout, reachable_cascade_a, reachable_a, GfirstOut, Gout, reachable_cascade_b, reachable_b, reachable_num_a, reachable_num_b, common_reachable);
		...
		int best_value_a=n*K, best_vertex_a, best_value_b=n*K, best_vertex_b, best_value_common = n*K, best_vertex_common;
		for(int j = 1; j<=n ;j++){
		...
		cout << "vertex "<< best_vertex_a <<" in team a will leave "<< (double)best_value_a/(double)K <<" vertices unbalanced on average"<< endl;
		cout << "vertex "<< best_vertex_b <<" in team b will leave "<< (double)best_value_b/(double)K <<" vertices unbalanced on average"<< endl;
		...
		for (int i=0; i<K; i++){
			...
			best_a_b_reachable += reachable_from_pair(best_vertex_a, best_vertex_b, GfirstOut, Gout, reachable_cascade_a, reachable_a, GfirstOut, Gout, reachable_cascade_b, reachable_b, reachable_num_a, reachable_num_b, common_reachable);
		}
		
		int best_value_a_b = best_a_b_reachable;
		
		cout << "vertex "<< best_vertex_a <<" in team a and vertex" << best_vertex_b<< " in team b will leave "<< (double)best_value_a_b/(double)K <<" vertices unbalanced on average"<< endl;
		
		if((best_value_a <= best_value_b && best_value_a <= best_value_common && best_value_a <= best_value_a_b) || (best_value_a <= best_value_b && main_iterations == NUM_TO_SELECT-1)){
		...
		else if((best_value_b <= best_value_a && best_value_b <= best_value_common && best_value_b <= best_value_a_b) || (best_value_b <= best_value_a && main_iterations == NUM_TO_SELECT-1)){
			for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
		...
		else if(best_value_common <= best_value_a && best_value_common <= best_value_b && best_value_common <= best_value_a_b){
			for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
		...
		else if(best_value_a_b <= best_value_a && best_value_a_b <= best_value_b && best_value_a_b <= best_value_common){
			for (map<string,int>::iterator it = user2ID.begin(); it != user2ID.end(); ++it ){
				if (it->second == best_vertex_a){
					cout << "--> seed  "<< it->first <<"  is selected in the first campaign and balances "<< (double)best_value_a/(double)K <<" vertices on average"<< endl;
				}
